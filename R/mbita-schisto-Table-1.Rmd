...
# Basic summary of measurements

Summarize number of DBS and stool samples by year.  
...
dtab_yr <- d3 %>%
  ...
  mutate(seaprev = sprintf("%1.1f", seapos / ndbs * 100),
         kkprev =  sprintf("%1.1f", kkpos/nkk * 100),
         desc = as.character(year)
  ) %>%
  dplyr::select(desc,ndbs,seaprev,nkk,kkprev)
...
```{r summary table}
knitr::kable(dtab_all,digits=0,escape=FALSE,
             caption="Table 1. Number of children and samples tested, stratified by age and by year.",
             col.names=c("","N<br>Blood<br>Samples","Sero-<br>prevalence<br>(%)","N<br>Stool<br>Samples","Kato-Katz<br>Prevalence<br>(%)")) %>%
