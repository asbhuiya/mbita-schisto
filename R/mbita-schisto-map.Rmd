...
fit_sea <- gam(cbind(sea_n,sea_N-sea_n) ~ s(lat,lon, k=-1,bs="gp") + s(dist_victoria,k=3) + s(elev,k=3) + s(tmin,k=3) + s(prec,k=3) , 
                   family="binomial",
                   data=dvil3,
                   method="REML",
                   select=TRUE)

#  s(lat,lon, k=-1,bs="gp")  + s(lat,lon, k=-1,bs="gp") + s(elev,k=3) + s(dist_victoria,k=3) + s(tmin,k=3) + s(prec,k=3) + yearf + s(vidf,bs="re",by=dummy)
...
# village-level random effects 
# (use of by=dummy enables us to zero-out random effects in predictions)
# + yearf + s(vidf,bs="re",by=dummy),

fit_kk <- gam(cbind(kk_n,kk_N-kk_n) ~ s(lat,lon, k=-1,bs="gp") + s(elev,k=3) + s(dist_victoria,k=3) + s(tmin,k=3) + s(prec,k=3),  
                   family="binomial",
                   data=dvil3,
                   method="REML",
                   select=TRUE)
...
xlon <- median(dvil2$lon)
ylat <- median(dvil2$lat)
