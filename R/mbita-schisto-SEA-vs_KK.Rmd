...
sscorr_foi_long <- bind_rows(sscorr_foi_seamfi,sscorr_foi_seaprev,sscorr_foi_kkmean,sscorr_foi_kkprev) %>%
  mutate(measure=factor(measure,levels=c("SEA MFI","SEA seroprevalence","Kato-Katz EPG","Kato-Katz prevalence")))
...
sscorr_foi_long_reps <- sscorr_foi1 %>%
  ...
  mutate(measure = case_when (
    ...
    outcome == "mean"   ~ "SEA MFI",
    outcome == "seroprev" ~ "SEA seroprevalence"
  ),
  measure = factor(measure, levels = c("SEA MFI","SEA seroprevalence","Kato-Katz EPG","Kato-Katz prevalence"))
         )
```
...
sscorfoip <- ggplot(data=sscorr_foi_long,aes(x=ss,y=cor)) +
  ...
  facet_grid(~measure)+
  geom_jitter(data=sscorr_foi_long_reps, aes(x = ss, y= cor), col = cbPalette[7],alpha=0.2,size =0.1,width=7)+
  geom_errorbar(aes(ymin=cor_lb,ymax=cor_ub))+
  geom_point(pch=19)+
  scale_fill_manual(values=cbPalette[c(7,7,7,7)])+
  scale_color_manual(values=cbPalette[c(7,7,7,7)])+
