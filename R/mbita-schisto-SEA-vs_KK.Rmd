...
Examine the influence of reducing the number of samples per village used to estimate seroprevalence and Kato-Katz prevalence, and determine whether estimates are still correlated with village-level force of infection estimated in the full sample. 

## Downsample seroprevalence estimates

Within each downsample, estimate mean log10 MFI for SEA, SEA seroprevalence, and Kato-Katz prevalence.
...
d4c <- d3c %>% 
  dplyr::select(vid,true_mu=meansea,true_seroprev=prevsea,true_kkprev=prevkk,lambda)
...
sscorfoip <- ggplot(data=sscorr_foi_long,aes(x=ss,y=cor,color=measure,fill=measure)) +
  ...
  facet_grid(~measure)+
  geom_pointrange(aes(ymin=cor_lb,ymax=cor_ub))+
  scale_fill_manual(values=cbPalette[c(2,3,6)])+
  scale_color_manual(values=cbPalette[c(4,3,2)])+
  scale_y_continuous(breaks=seq(0,1,by=0.1))+
  scale_x_continuous(breaks=seq(20,200,by=40))+
  coord_cartesian(ylim=c(0.7,1),xlim=c(20,200))+
  labs(y=expression(paste("correlation with community seroconversion rate (",lambda,")")),x="sample size per community used to estimate the average") +
  theme_minimal()+
