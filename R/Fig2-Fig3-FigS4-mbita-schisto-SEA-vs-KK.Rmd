...
foi_ests <- foreach(comi=levels(d3$vid),.combine=rbind) %do% {
  ...
  gsum <- summary(gfit)
  lambda <- as.numeric(exp(gfit$coefficients[1]))
  log_lambda_se  <- sqrt(gsum$cov.unscaled[1,1])
...
```{r cluster means vs kk, fig.width=6, fig.height=12}
...
sp_epg_plot <- ggplot(data=d3c,aes(x=meankk,y=prevsea)) +
  ...
  labs(x="",y="SEA seroprevalence (%)",tag = "B") +
...
foi_epg_plot <- ggplot(data=d3c,aes(x=meankk,y=lambda)) +
  ...
  annotate("text",x=0.05,y=0.625,label=paste("rho ==", deparse(cormukkfoi_print)),parse=TRUE, hjust=0)+
