---
title: "Fine-scale heterogeneity in Schistosoma mansoni force of infection measured through serologic surveillance"
...
pspdist <- ggplot(data=fit_sea_dist_ci,aes(x=dist_victoria/1000)) +
  ...
  geom_point(data=filter(dvil3,dist_victoria<1500),aes(x=dist_victoria/1000,y=sea_prev),alpha=0.8,color=cbPalette[2]) +
  # village means, >1500 m
  geom_point(data=filter(dvil3,dist_victoria>=1500),aes(x=dist_victoria/1000,y=sea_prev),alpha=1,color=vircols[3]) +
  scale_y_continuous(breaks=seq(0,1,by=0.2),labels=sprintf("%1.0f",seq(0,1,by=0.2)*100))+
  scale_x_continuous(breaks=0:5)+
  # scale_color_manual(values=pcols)+
...
pfoidist <- ggplot(data=dvil3,aes(x=dist_victoria/1000,y=lambda,ymin=lambda_lb,ymax=lambda_ub)) +
  ...
  geom_point(data=filter(dvil3,dist_victoria<1500),alpha = 0.8, pch=19, bg=cbPalette[7], color = cbPalette[2], size = 1.75)+
  # village mean FOI for villages >1500m
  geom_pointrange(data=filter(dvil3,dist_victoria>=1500),alpha=1,color=vircols[3],size = 0.3) +
